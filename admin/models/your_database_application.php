...
/**
 * Item Model for your_database_application.
 *
 * @package     Datagrill
 * @subpackage  Models
 */
class DatagrillModelYour_Database_Application extends JModelAdmin
{
	/**
	 * @var        string    The prefix to use with controller messages.
	 * @since   1.6
	 */
	protected $text_prefix = 'COM_DATAGRILL';
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_datagrill.your_database_application';
	...
	public function appPathExists()
	{
		$data = $this->loadFormData();
 		$path=$data->get('apppath');
		return $path;
	}
	...
	public function setAppPath()
	{
		$data = $this->loadFormData();
		$id=$data->get('id');
		$table=$this->getTable('Your_Database_Application');
 		$table->load($id);
		$itemname=$table->get('greeting');
    		$appPath=JPATH_COMPONENT_SITE.'/apps/'.urlencode($id).'_'.urlencode($itemname);
		$appurl='components/com_datagrill/apps/'.urlencode($id).'_'.urlencode($itemname);
    		if (!file_exists($appPath))  {
       			mkdir($appPath, 0777, true);
    			}
		$templPath=$appPath.'/templates_c';
    		if (!file_exists($templPath))  {
			mkdir ($templPath, 0777, true);
    			}

		$table->set('apppath',$appPath);
		$table->set('appurl',$appurl);
		$table->store($id);
		$data->store;
		return $appPath;
