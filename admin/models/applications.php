...
/**
 * List Model for applications.
 *
 * @package     Datagrill
 * @subpackage  Models
 */
class DatagrillModelApplications extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				'a.title', 'title',
				'a.catid', 'catid', 'category_id', 'category_title','ordering', 'state', 'title', 'dbname'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__datagrillapplication AS a');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('a.title LIKE' . $s . ' OR a.dbname LIKE' . $s );
			}
		}
